...
| 物料形态 | 参照 | 存 | 渲染 | 现有例子 |
|----------|------|-----|------|----------|
| **图/占一块**（宽度即内容） | 文档列宽 | `width` 列宽% | CSS `width:%` | logo · 行内图片 |
...
- **列宽%（图/块）** → 复用 `decos/_shared/page-size.ts` 的 `storeResizedWidth(block, px)`（拖拽落点 px → 量 `.editor-container` 内容宽 → 存 `props.width`%）；模板 `[style.width]="props.width ? props.width + '%' : null"`；控件用框架 `block-resizer`。**换算别自己抄。**
...
1. **渲染 `%` 参照的盒子必须 = 换算时量的盒子**（`.editor-container` 内容宽）。中间 wrapper 别夹更窄的 padding/max-width；inline 用满宽 `display` 让 `%` 透传到列，否则「存 42% 渲出来不是列宽的 42%」。
