...
export interface DecoBlockRef {
  flavour?: string
  props?: { x?: number; y?: number; z?: number; float?: 'left' | 'right'; align?: 'left' | 'center' | 'right'; mb?: number; ml?: number; mr?: number }
  updateProps(p: { x?: number | null; y?: number | null; z?: number | null; deg?: number | null; float?: 'left' | 'right' | null; align?: 'left' | 'center' | 'right' | null; mb?: number | null; ml?: number | null; mr?: number | null }): void   // null = 删该 prop（层级留空→默认；切独占→清 float/x/y）
}
 ...
 * 不走框架 `selection.selectBlock`：它对 absolute 脱流块建的 DOM Range 会落到相邻段落 → 选区/内容错乱。
 */
