...
/** 一份「模板」= 内容快照 + 整页背景图。`/template` 设计页存它，`/template/use` 使用页读它。 */
...
export class TemplateStore {
  ...
  save(payload: TemplatePayload): void {
    ...
    try { localStorage.setItem(LS_KEY, JSON.stringify(payload)) } catch { /* 超配额等：忽略，内存态仍在 */ }
  }
  ...
  load(): TemplatePayload | null {
    ...
    try {
      ...
      if (raw) this.payload = JSON.parse(raw) as TemplatePayload
    } catch { /* 解析失败：当作没有 */ }
    return this.payload
  }
