 ...
 * 「使用模版」把当前模板（snapshot + 整页背景）存入 TemplateStore，跳转 /template/use 使用页（不再弹预览窗）。
 */
@Component({
  ...
  imports: [TemplateEditSurfaceComponent, DecoInsertPanelComponent, DecoLayoutPanelComponent, TemplateDebugAsideComponent, RouterLink],
  template: `
    <div class="tpl-page">
      ...
      <div class="tpl-page__body">
        ...
        <main class="tpl-page__editor">
          <template-edit-surface (ready)="editDoc.set($event)"></template-edit-surface>
        </main>
        <aside class="tpl-page__panel tpl-page__panel--right" [class.is-collapsed]="!panelOpen()">
          @if (panelOpen()) {
            ...
            @if (editDoc(); as d) {
              ...
              <deco-insert-panel [doc]="d" (pickPageBackground)="editSurface?.pickPageBackground()"></deco-insert-panel>
            }
          } @else {
...
export class TemplatePageComponent {
  ...
  /** 存当前模板（内容 + 整页背景）→ 跳使用页。surface 自己持久化，这里 flush 一次保证最新。 */
  useTemplate(): void {
