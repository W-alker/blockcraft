...
import {remarkGfm} from '../adapters/markdown-adapter/gfm';
...
describe('MarkdownAdapter', () => {
  ...
  describe('inline formatting', () => {
    ...
    it('round-trips mention embeds through readable URN links', async () => {
      const snapshot = createRootSnapshot([
      ...
      expect(markdown).toContain(
        'Hello [@Alice](urn:blockcraft:mention:user:user-1',
      );
      const imported = await adapter.toBlockSnapshot(markdown);
      const paragraph = imported.children[0] as IBlockSnapshot;
      expect((paragraph.children[1] as DeltaInsert).insert)
        .toEqual({mention: 'Alice'});
      expect((paragraph.children[1] as DeltaInsert).attributes)
  ...
  describe('media blocks', () => {
    ...
    it('keeps media as readable typed links in the BlockCraft profile', async () => {
      const blockcraftAdapter = new MarkdownAdapter(
        new TestDocFileService(),
      ...
      const fixtures = [
        ...
        createVoidSnapshot('audio-1', 'audio', {
          url: 'https://cdn.example.com/theme.ogg',
          name: 'Theme song',
          sourceType: 'link',
          size: 2048,
        }),
      ...
      for (const fixture of fixtures) {
        const markdown = await blockcraftAdapter.toMarkdown(
          createRootSnapshot([fixture])
        );
        const imported = await adapter.toBlockSnapshot(markdown);
        const media = (imported.children as IBlockSnapshot[])[0];

        expect(markdown).toContain(`"blockcraft:${fixture.flavour}"`);
        expect(markdown).not.toContain(`::bc-${fixture.flavour}`);
        expect(media.flavour).toBe(fixture.flavour);
        expect(media.props['url']).toBe(fixture.props['url']);
        expect(media.props['name']).toBe(fixture.props['name']);
        expect(media.props['sourceType']).toBe('link');
        expect(media.props['poster']).toBeUndefined();
        expect(media.props['width']).toBeUndefined();
        expect(media.props['size']).toBeUndefined();
      }
    });
  ...
  describe('code blocks', () => {
    ...
    it('round-trips a mermaid fence as one mermaid block', async () => {
      const source = '```mermaid\ngraph TD\n  A --> B\n```\n'
      const snapshot = await adapter.toBlockSnapshot(source)
      const rootChildren = snapshot.children as IBlockSnapshot[]
      const mermaid = rootChildren[0]

      expect(rootChildren.map(child => child.flavour)).toEqual(['mermaid'])
      expect(mermaid.props['mode']).toBe('graph')
      const textarea = (mermaid.children as IBlockSnapshot[])[0]
      expect(textarea.flavour).toBe('mermaid-textarea')
      expect((textarea.children as DeltaInsert[]).map(delta => delta.insert).join(''))
        .toBe('graph TD\n  A --> B')

      const markdown = await adapter.toMarkdown(snapshot)
      expect(markdown).toBe(source)
      expect(markdown).not.toContain('bc-mermaid')
    })

    it('keeps mermaid as a standard fence in the blockcraft profile', async () => {
      const blockcraftAdapter = new MarkdownAdapter(
        new TestDocFileService(),
        new Map([[MARKDOWN_ADAPTER_PROFILE_CONFIG, 'blockcraft']]),
        BUNDLED_ADAPTER_REGISTRY,
      ...
      const source = '```mermaid\nsequenceDiagram\n  Alice->>Bob: Hello\n```\n'
      const snapshot = await blockcraftAdapter.toBlockSnapshot(source)
      const markdown = await blockcraftAdapter.toMarkdown(snapshot)

      expect((snapshot.children as IBlockSnapshot[]).map(child => child.flavour))
        .toEqual(['mermaid'])
      expect(markdown).toBe(source)
      expect(markdown.match(/sequenceDiagram/g)?.length).toBe(1)
      expect(markdown).not.toContain('::bc-mermaid')
    })

    it('accepts the common mermiad typo and canonicalizes it on export', async () => {
      ...
      expect(markdown).not.toContain('```mermiad')
    })
  });
  ...
  describe('dividers', () => {
    ...
    it('keeps portable Markdown standard and intentionally drops Divider props', async () => {
      const snapshot = createRootSnapshot([
        createVoidSnapshot('divider-portable', 'divider', {
          style: 'double',
          length: 'long',
          text: 'OR',
        }),
      ...
      const markdown = await adapter.toMarkdown(snapshot)
      const imported = await adapter.toBlockSnapshot(markdown)
      const divider = (imported.children as IBlockSnapshot[])[0]

      expect(markdown).toMatch(/\*\*\*|---|___/)
      expect(markdown).not.toContain('bc-divider')
      expect(divider.flavour).toBe('divider')
      expect(divider.props).toEqual({})
    })

    it('keeps Divider as a thematic break in the BlockCraft profile', async () => {
      const blockcraftAdapter = new MarkdownAdapter(
        new TestDocFileService(),
        new Map([[MARKDOWN_ADAPTER_PROFILE_CONFIG, 'blockcraft']]),
        BUNDLED_ADAPTER_REGISTRY,
      ...
      const snapshot = createRootSnapshot([
        createVoidSnapshot('divider-blockcraft', 'divider', props),
      ])

      const markdown = await blockcraftAdapter.toMarkdown(snapshot)
      const imported = await blockcraftAdapter.toBlockSnapshot(markdown)
      const divider = (imported.children as IBlockSnapshot[])[0]
    ...
    it('accepts a prior private Divider directive but exports a thematic break', async () => {
      const blockcraftAdapter = new MarkdownAdapter(
        new TestDocFileService(),
        new Map([[MARKDOWN_ADAPTER_PROFILE_CONFIG, 'blockcraft']]),
        BUNDLED_ADAPTER_REGISTRY,
      )
      const imported = await adapter.toBlockSnapshot(':::bc-divider\n:::\n')
      const divider = (imported.children as IBlockSnapshot[])[0]
      const markdown = await blockcraftAdapter.toMarkdown(imported)
  ...
  describe('mixed content', () => {
    const textBoxSnapshot = () => createRootSnapshot([{
      id: 'text-box-1',
      flavour: 'text-box',
    ...
    it('keeps a text-box envelope in the default hybrid profile', async () => {
      const markdown = await adapter.toMarkdown(textBoxSnapshot());

      expect(markdown).toContain(':::bc-text-box');
      expect(markdown).toContain(':::bc-text-box\n\n---');
      expect(markdown).toContain('width: 320');
      expect(markdown).toContain('p: [8,12]');
      expect(markdown).toMatch(/p: \[8,12\][\s\S]*\n---\n\n/);
      expect(markdown).toContain('Text box **content**');
      expect(markdown).toMatch(/[*-] List item/);
      expect(markdown.trimEnd()).toMatch(/List item\n\n:::$/);
    ...
    it('flattens text-box children in the explicit portable profile', async () => {
      const portableAdapter = new MarkdownAdapter(
        new TestDocFileService(),
        new Map([[MARKDOWN_ADAPTER_PROFILE_CONFIG, 'portable']]),
        BUNDLED_ADAPTER_REGISTRY,
      );
      const markdown = await portableAdapter.toMarkdown(textBoxSnapshot());
    ...
    it('imports authored YAML metadata without creating a code child', async () => {
      const source = [
        ':::bc-text-box',
        '',
        '---',
      ...
      const imported = await adapter.toBlockSnapshot(source);
      const textBox = (imported.children as IBlockSnapshot[])[0];
    ...
    it('does not import the former percent-encoded props attribute', async () => {
      const imported = await adapter.toBlockSnapshot([
        ':::bc-text-box{props="%7B%22width%22%3A999%7D"}',
      ...
      expect(textBox.flavour).toBe('text-box');
      expect(textBox.props['width']).toBeUndefined();
    });

    it('does not interpret directive-like YAML inside fenced code', async () => {
      const source = [
        '```markdown',
        ':::bc-text-box',
        '',
      ...
      const imported = await adapter.toBlockSnapshot(source);
      const blocks = imported.children as IBlockSnapshot[];

      expect(blocks.map(block => block.flavour)).toEqual(['code']);
      expect(JSON.stringify(blocks[0].children)).toContain(':::bc-text-box');
      expect(JSON.stringify(blocks[0].children)).toContain('width: 999');
    });
