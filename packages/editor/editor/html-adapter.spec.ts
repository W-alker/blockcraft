...
describe('HtmlAdapter', () => {
  ...
  describe('code, list, and formula identity', () => {
    it('round-trips the code language through portable HTML', async () => {
      const code: IBlockSnapshot = {
        id: 'code-typescript',
        flavour: 'code',
        nodeType: BlockNodeType.editable,
        props: {depth: 0, lang: 'TypeScript'},
        meta: {},
        children: [{insert: 'const answer: number = 42'}],
      }

      const html = await adapter.toHtml(createRootSnapshot([code]))
      const imported = await adapter.toBlockSnapshot(html)
      const restored = (imported.children as IBlockSnapshot[])[0]

      expect(html).toContain('data-bc-lang="TypeScript"')
      expect(html).toContain('class="language-typescript"')
      expect(restored.flavour).toBe('code')
      expect(restored.props['lang']).toBe('TypeScript')
    })

    it('round-trips a checked todo without degrading it to bullet', async () => {
      const todo: IBlockSnapshot = {
        id: 'todo-checked',
        flavour: 'todo',
        nodeType: BlockNodeType.editable,
        props: {depth: 0, checked: true},
        meta: {},
        children: [{insert: '完成适配'}],
      }

      const html = await adapter.toHtml(createRootSnapshot([todo]))
      const imported = await adapter.toBlockSnapshot(html)
      const restored = (imported.children as IBlockSnapshot[])[0]

      expect(html).toContain('type="checkbox"')
      expect(restored.flavour).toBe('todo')
      expect(restored.props['checked']).toBeTrue()
      expect(restored.children).toEqual([{insert: '完成适配', attributes: {}}])
    })

    it('lets the formula matcher consume its HTML envelope before paragraph', async () => {
      const formula = createVoidSnapshot('formula-html', 'formula', {
        latex: 'E = mc^2',
      })

      const html = await adapter.toHtml(createRootSnapshot([formula]))
      const imported = await adapter.toBlockSnapshot(html)
      const children = imported.children as IBlockSnapshot[]

      expect(children.length).toBe(1)
      expect(children[0].flavour).toBe('formula')
    ...
    it('round-trips Divider props through a bounded hr envelope', async () => {
      ...
      const divider = createVoidSnapshot('divider-html', 'divider', props)

      const html = await adapter.toHtml(createRootSnapshot([divider]))
      const imported = await adapter.toBlockSnapshot(html)
      const restored = (imported.children as IBlockSnapshot[])[0]
    ...
    it('keeps an ordinary external hr as a default Divider', async () => {
      const imported = await adapter.toBlockSnapshot('<p>Before</p><hr><p>After</p>')
      const divider = (imported.children as IBlockSnapshot[])
        .find(child => child.flavour === 'divider')!
