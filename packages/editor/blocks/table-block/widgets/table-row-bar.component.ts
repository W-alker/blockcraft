...
export class TableRowBarComponent {
  ...
  onMouseDown(idx: number, event: MouseEvent) {
    ...
    // Preserve an existing multi-row selection when the mousedown lands on a
    // handle inside that range — so dragging moves the whole selection, not
    // just the clicked row. Click outside the range resets to single.
