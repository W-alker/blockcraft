        ...
        >
          <div class="text-box-block__content">
            <p
              ...
              style="--bc-block-sb: 12px; --bc-block-leading-sb: 12px; --bc-block-sa: 40px"
            >
              一
            </p>
        ...
        >
          <div class="text-box-block__content">
            ...
            <div id="vertical-todo" class="todo-block" data-block-id>
              <button class="todo-block-button" contenteditable="false">
                □
              </button>
              <div class="edit-container" style="inline-size: 72px">
                待办文本需要换成多列才能验证首列对齐
...
@Component({
  ...
  template: `
    <div data-blockcraft-root="true">
      <div class="text-box-block" [style]="insetStyle">
        <div
          id="design-size"
          class="text-box-block__surface"
...
class TextBoxArtworkFrameHarness {
  private readonly insets = getTextBoxArtwork(
    getTextBoxPreset("office-banded").props.artwork,
  )!.textInsets;
...
describe("TextBoxBlockSchema", () => {
  ...
  it("restores an editable child surface inside non-editable object chrome", async () => {
    ...
    try {
      ...
      expect(objectHandle?.getAttribute("aria-label")).toBe("选中文本框并拖动");
      expect(
        objectHandle?.hasAttribute("data-bc-selection-interaction-ignore"),
      ).toBeTrue();
      expect(
        objectHandle?.hasAttribute("data-bc-placement-pick-ignore"),
      ).toBeTrue();
      ...
      expect(
        frameHitTarget?.hasAttribute("data-bc-selection-interaction-frame"),
      ).toBeTrue();
      ...
      expect(
        fixture.nativeElement.querySelector(".text-box-block__object-handle"),
      ).toBeNull();
    } finally {
      fixture.destroy();
      yDoc.destroy();
  ...
  it("normalizes and persists shape adjustment values for callout text boxes", () => {
    const props = normalizeTextBoxProps({
    ...
    expect(props.adjustments).toEqual({ tailX: 170, tailY: 0 });
    expect(snapshot.props.adjustments).toEqual({ tailX: 170, tailY: 0 });
    expect(
      normalizeTextBoxProps({ adjustments: { tailX: Number.NaN } })
        .adjustments,
    ).toBeUndefined();
  });
  ...
  it("falls back instead of coercing missing dimensions to minimums", () => {
    expect(
      TextBoxBlockSchema.createSnapshot("", {
  ...
  it("owns a closed rich-text child contract and fixed model height", () => {
    ...
    const estimateHeight =
      TextBoxBlockSchema.metadata.virtualization?.estimateHeight;
