...
describe('rehostYneAttachments', () => {
  ...
  it('does not update a block removed while upload is pending', async () => {
    const block = {setInitProps: jasmine.createSpy('setInitProps')};
    let alive = true;
    let finishUpload!: (value: {url: string; size: number; name: string}) => void;
    const uploaded = new Promise<{url: string; size: number; name: string}>(resolve => { finishUpload = resolve; });
    let started!: () => void;
    const uploadStarted = new Promise<void>(resolve => { started = resolve; });
    const doc = {
      vm: {get: () => alive ? {instance: block} : undefined},
      injector: {get: () => ({uploadAttachment: () => { started(); return uploaded; }})},
      logger: {warn() {}},
    };
    spyOn(window, 'fetch').and.resolveTo(new Response('file'));
    const rehost = rehostYneAttachments(doc as unknown as BlockCraft.Doc, [{
      snapshot: {id: 'later-removed'} as IBlockSnapshot,
      url: 'https://note.youdao/file.csv', fileName: 'file.csv', fileLength: 4,
    }]);
    await uploadStarted;
    alive = false;
    finishUpload({url: 'https://host/file.csv', name: 'file.csv', size: 4});
    await rehost;
    expect(block.setInitProps).not.toHaveBeenCalled();
  });
});
