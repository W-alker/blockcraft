// packages/editor/adapters/yne-adapter/index.spec.ts
...
describe('parseYneClipboard', () => {
  ...
  it('wraps converted blocks in a root snapshot', () => {
    ...
    expect(res!.snapshot.flavour).toBe('root');
    expect(rootChildren(res!.snapshot).length).toBe(2);
    expect(res!.deferredAttachments).toEqual([]);
  });
  ...
  it('collects deferred attachments by snapshot reference', () => {
    ...
    const res = parseYneClipboard(fakeState({ [YNE_JSON_MIME]: json }) as any, fakeDoc);
    expect(res!.deferredAttachments.length).toBe(1);
    expect(res!.deferredAttachments[0].snapshot).toBe(rootChildren(res!.snapshot)[0]);
  });
  ...
  it('parses the image map from text/yne-image-json', () => {
    ...
    expect(rootChildren(res!.snapshot)[0].props['src']).toBe('blob-local:image.png');
  });
});
