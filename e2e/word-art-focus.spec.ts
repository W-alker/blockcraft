...
test('WordArt object handle, format panel, and Escape keep object focus stable', async ({
  ...
  await expect(formatPanel.locator('.object-format__scroll')).toHaveCSS('overflow-x', 'hidden')
  await expect(formatPanel.locator('.object-format__scroll')).toHaveCSS('overflow-y', 'auto')
