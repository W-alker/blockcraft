export async function printToPdf(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    iframe.onload = async () => {
      ...
      for (const s in document.styleSheets) {
        const element = document.styleSheets[s];
        try {
          for (const cssRuleName in element.cssRules) {
            const cssRule = element.cssRules[cssRuleName];
      ...
      const allCanvas = Array.from(rootElement.getElementsByTagName('canvas'))
      let canvasKey = 1;
      ...
      const allImportedCanvas = Array.from(importedRoot.getElementsByTagName('canvas'))
